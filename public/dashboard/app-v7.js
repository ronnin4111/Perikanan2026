...
function cleanBlueApp() {
    return {
        ...
        async initApp() {
            ...
            // Track scroll for mobile navigation
            window.addEventListener('scroll', () => {
                this.scrollY = window.scrollY;
                this.updateMobileActiveTab();
            });
        ...
        // Mobile Active Tab Update based on Scroll
        updateMobileActiveTab() {
            ...
            const mapTop = mapSection ? mapSection.getBoundingClientRect().top + window.scrollY - 100 : 0;
            const tableTop = tableSection ? tableSection.getBoundingClientRect().top + window.scrollY - 100 : 0;

            if (this.scrollY < mapTop) {
                this.mobileActiveTab = 'dashboard';
            } else if (this.scrollY >= mapTop && (!tableSection || this.scrollY < tableTop)) {
                this.mobileActiveTab = 'map';
            } else if (tableSection && this.scrollY >= tableTop) {
                this.mobileActiveTab = 'data';
            }
        },
        ...
        scrollToSection(sectionId) {
            this.mobileActiveTab = sectionId;
            this.$nextTick(() => {
                let element;
                switch (sectionId) {
                    case 'dashboard':
...
window.addEventListener('DOMContentLoaded', function() {
    ...
    setTimeout(() => {
        ...
        if (appElement && appElement._x_dataStack) {
            ...
            if (app) {
                console.log('✅ Alpine app instance found');

                // Check authentication status
                const isAuth = window.Auth ? window.Auth.isAuthenticated() : false;
                console.log('🔐 Auth status on load:', isAuth);

                if (isAuth) {
                    // User already authenticated from session
                    ...
                    setTimeout(() => app.initApp(), 50);
                } else {
                    // Show password gate
                    console.log('🔐 Waiting for user login...');
                    app.isLoading = false; // Don't show loading for unauthenticated users
                }
            } else {
                console.error('❌ Alpine app instance not found');
            }
        } else {
            console.error('❌ App element or Alpine data stack not found');
        }
    }, 200);
});
